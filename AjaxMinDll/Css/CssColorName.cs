...
namespace Microsoft.Ajax.Utilities
{
    ...
    // entire table of all the color name data
    internal static class ColorNameData
    {
        internal static ColorName[] ColorName { get; private set; }
    ...
    /// <summary>
    /// Base class for all queries related to color names.
    /// </summary>
    internal abstract class ColorQuery
    {
        internal abstract Dictionary<string, string> Results { get; }
    }

    /// <summary>
    /// Slicer for all views based on color name data. Each of them is static, 
    /// so each slice type will be initialized just once.
    /// </summary>
    /// <typeparam name="T">Query type.</typeparam>
    internal static class ColorSlice<T> where T : ColorQuery, new()
    {
        internal static Dictionary<string, string> Data { get; private set; }
    ...
    internal class AllColorNames : ColorQuery
    {
        internal override Dictionary<string, string> Results
        {
            ...
            {
                /*
                var query = from colorName in ColorNameData.ColorName
                            select new { Name = colorName.Name, Hex = colorName.Hex };

                return query.ToDictionary(p => p.Name, p => p.Hex);
                */
                Dictionary<string, string> results = new Dictionary<string, string>();
                foreach (ColorName colorName in ColorNameData.ColorName)
                {
                    results.Add(colorName.Name, colorName.Hex);
                }
                return results;
            }
        }
        ...
        internal override Dictionary<string, string> Results
        {
            get
            {
                ...
                */
                ...
                foreach (ColorName colorName in ColorNameData.ColorName)
                {
                    if ((colorName.Strict && colorName.Name.Length > colorName.Hex.Length)
                        || !colorName.Strict)
                    {
                        results.Add(colorName.Name, colorName.Hex);
                    }
        ...
        internal override Dictionary<string, string> Results
        {
            get
            {
                ...
                */
                ...
                foreach (ColorName colorName in ColorNameData.ColorName)
                {
                    if (colorName.Name.Length > colorName.Hex.Length)
                    {
                        results.Add(colorName.Name, colorName.Hex);
                    }
                }
        ...
        internal override Dictionary<string, string> Results
        {
            get
            {
                ...
                var query = from colorName in ColorNameData.ColorName
                            ...
                            select new { Hex = colorName.Hex, Name = colorName.Name };

                return query.ToDictionary(p => p.Hex, p => p.Name);
                */
                Dictionary<string, string> results = new Dictionary<string, string>();
                foreach (ColorName colorName in ColorNameData.ColorName)
                {
