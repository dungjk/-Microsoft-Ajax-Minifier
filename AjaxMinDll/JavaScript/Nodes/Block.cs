...
namespace Microsoft.Ajax.Utilities.JavaScript.Nodes
{
    ...
    public sealed class Block : AstNode
    {
        ...
        public AstNode this[int index]
        {
            ...
            set
            {
                if (value != m_list[index])
                {
                    ...
                    if (value == null)
                    {
                        // setting to null is the same as removing
                        m_list.RemoveAt(index);
                    }
                    else
                    {
                        Block newBlock = value as Block;
                        if (newBlock != null)
                        {
                            ...
                            m_list.RemoveAt(index);
                            foreach (var newNode in newBlock.m_list)
                            {
                                newNode.Parent = this;
                            }
                            m_list.InsertRange(index, newBlock.m_list);
                        }
                        else
                        {
                            // not a block -- just slap it in there
                            m_list[index] = value;
                            value.Parent = this;
                        }
                    }
        ...
        public override LexicalEnvironment EnclosingLexicalEnvironment
        {
            get
            {
                // if we have an execution context, return it; otherwise just
                // continue up the chain
                return LexicalEnvironment ?? base.EnclosingLexicalEnvironment;
            }
        }
        ...
        public override LexicalEnvironment  EnclosingVariableEnvironment
        {
            get
            {
                // if we have an execution context, return it; otherwise just
                // continue up the chain
                return VariableEnvironment ?? base.EnclosingVariableEnvironment;
            }
        }
        ...
        public override bool IsExpression
        {
            get
            {
                ...
                // otherwise it isn't.
                //
                // TODO: if there are no statements -- empty block -- then is an expression?
        ...
        public override IEnumerable<AstNode> Children
        {
            get
            {
                ...
                // thisis to keep us from accidentally skipping over things in some of
                // those visitors that delete themselved or other nodes and therefore 
                // mess up the parent's iteration counter
        ...
        public void RemoveLast()
        {
            if (m_list.Count > 0)
            {
                var lastNode = m_list[m_list.Count - 1];
                if (lastNode != null && lastNode.Parent == this)
                {
