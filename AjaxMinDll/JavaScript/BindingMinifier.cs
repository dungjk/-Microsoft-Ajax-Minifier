...
{
    internal class BindingMinifier
    {
        ...
        private static string s_varPartLetters = "gntirufeoshclavypwbkd";//"$abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_";//"ntirufeoshclavypwbkdgjmqxz";//

        //internal BindingMinifier(Dictionary<string, string> avoidNames)
        //{
        //    // just use the dictionary we were passed
        //    m_skipNames = avoidNames;
        //}

        internal BindingMinifier(IList<Reference> noCollideList)
        {
            if (noCollideList != null)
            {
                // empty hashtable
                m_skipNames = new Dictionary<string, string>(noCollideList.Count);

                // walk through all the items in verboten
                foreach (var reference in noCollideList)
                {
                    string name;
                    Binding binding;
                    if (reference.Base != null && reference.Base.TryGetBinding(reference.Name, out binding))
                    {
                        // use the ToString method to get the proper name
                        name = binding.ToString();
                    }
                    else
                    {
                        // couldn't resolve -- just use the reference name
                        name = reference.Name;
                    }

                    m_skipNames.Add(name, name);
                }
            }
        }
        ...
        internal string NextName()
        {
            ...
            while ((m_skipNames != null && m_skipNames.ContainsKey(name)) || JSScanner.IsKeyword(name, true));
            return name;
        }
        ...
        public static string GenerateNameFromNumber(int num)
        {
            ...
            var name = s_varFirstLetters[num % s_varFirstLetters.Length].ToString();
            num /= s_varFirstLetters.Length;

            // if we need more than one number, generate them now
            while (num > 0)
            {
                name = s_varPartLetters[num % s_varPartLetters.Length] + name;
    ...
    /// Lastly, alphabetize.
    /// </summary>
    internal class RenameBindingComparer : IComparer<Binding>
    {
        // singleton instance
        public static readonly IComparer<Binding> Instance = new RenameBindingComparer();
        // never instantiate outside this class
        private RenameBindingComparer() { }

        #region IComparer<Binding> Members
        ...
        public int Compare(Binding left, Binding right)
        {
            ...
            var leftIsArg = left.Category == BindingCategory.Argument;
            var rightIsArg = right.Category == BindingCategory.Argument;

            if (leftIsArg && rightIsArg)
            {
            ...
            // with smaller names)
            var delta = right.ReferenceCount - left.ReferenceCount;
            if (delta == 0)
            {
                delta = left.Position - right.Position;
                if (delta == 0)
                {
                    delta = string.CompareOrdinal(left.Name, right.Name);
                }
            }
            return delta;
        }
