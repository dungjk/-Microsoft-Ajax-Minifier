...
{
    internal class MinifyVisitor : TreeVisitor
    {
        ...
        private Stack<LabelInfo> m_labelStack;

        private Stack<LexicalEnvironment> m_lexicalEnvironment;
        private Stack<LexicalEnvironment> m_variableEnvironment;

        public static void Minify(Block program, JSParser parser)
        {
            var visitor = new MinifyVisitor(parser);
            program.Accept(visitor);
        }

        private MinifyVisitor(JSParser parser)
        {
            // save the parser
            m_parser = parser;

            // push the global environment onto both the lexical and variable stacks
            m_lexicalEnvironment = new Stack<LexicalEnvironment>();
            m_lexicalEnvironment.Push(parser.GlobalScope);

            m_variableEnvironment = new Stack<LexicalEnvironment>();
            m_variableEnvironment.Push(parser.GlobalScope);
        }
        ...
        private void CombineExpressions(Block node)
        {
            ...
            for (var ndx = node.Count - 1; ndx > 0; --ndx)
            {
                ...
                if (node[ndx - 1].IsExpression)
                {
                    ...
                    else if ((forNode = node[ndx] as ForStatement) != null)
                    {
        ...
        private static int FindLastStatement(Block node)
        {
        ...
        public override void Visit(Block node)
        {
            if (node != null)
            {
                try
                {
                    ...
                    {
                        // push the environment on the lexical stack
                        m_lexicalEnvironment.Push(node.LexicalEnvironment);

                        // if we are removing unneeded code (and this isn't the global environment)
                        // then process the lexical environment now, before we bother minifying all the
                        // code *within* those functions
                        // WAIT... if we aren't doing the global environment, then we won't ever have any
                        // function declarations at the block-scope level because functions will be defined
                        // at the function-scope level.
                    }
                    if (node.VariableEnvironment != null)
                    {
                        // push the environment on the variable stack
                        m_variableEnvironment.Push(node.VariableEnvironment);
                    }
                        ...
                        for (var ndx = 0; ndx < node.Count; ++ndx)
                        {
                                ...
                                || node[ndx] is ContinueStatement)
                            {
                                ...
                                for (var ndxRemove = node.Count - 1; ndxRemove > ndx; --ndxRemove)
                                {
                                    ...
                                    {
                                        var varStatement = node[ndxRemove] as VarStatement;
                        ...
                        && m_parser.Settings.IsModificationAllowed(TreeModifications.IfConditionReturnToCondition))
                    {
                        ...
                        var lastStatementIndex = FindLastStatement(node);
                        var ifNode = node[lastStatementIndex] as IfStatement;
                        if (ifNode != null 
                            && ifNode.FalseBlock == null
                            ...
                            && (returnNode = ifNode.TrueBlock[0] as ReturnStatement) != null)
                        {
                            ...
                            if (returnNode.Operand == null)
                            {
                                ...
                                node[lastStatementIndex] = ifNode.Condition;
                            }
                            else if (returnNode.Operand.IsExpression)
                            {
                                ...
                                node[lastStatementIndex] = new ReturnStatement(ifNode.Context, m_parser, conditional);
                    ...
                    if (m_parser.Settings.IsModificationAllowed(TreeModifications.MoveVarIntoFor))
                    {
                        ...
                        for (int ndx = node.Count - 1; ndx > 0; --ndx)
                        {
                            ...
                            ForStatement forNode;
                            var previousVar = node[ndx - 1] as VarStatement;
                            if (previousVar != null && (forNode = node[ndx] as ForStatement) != null)
                            {
                                // and see if the forNode's initializer is empty
                                if (forNode.Initializer != null)
                                {
                                    // not empty -- see if it is a Var node
                                    VarStatement varInitializer = forNode.Initializer as VarStatement;
                    ...
                    // get the index of the last statement (not including comments or function declarations)
                    // and see if the last statement is a return statement
                    var indexLastStatement = FindLastStatement(node);
                    ReturnStatement lastReturn;
                    if (indexLastStatement >= 0
                        && (lastReturn = node[indexLastStatement] as ReturnStatement) != null)
                    {
                        ...
                        var indexPrevious = PreviousStatementIndex(node, indexLastStatement);
                        ...
                        {
                            var varStatement = node[indexPrevious] as VarStatement;
                            if (varStatement != null)
                            {
                                ...
                                if ((varDecl = varStatement[varStatement.Count - 1]).Initializer != null
                                    ...
                                    && varDecl.Binding.ReferenceCount == 1)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        lastReturn.Operand = varDecl.Initializer;
                                        varStatement.RemoveAt(varStatement.Count - 1);
                                    }
                                }
                            }
                        }
                            ...
                            && previousIf.FalseBlock == null)
                        {
                                ...
                                if (lastReturn.Operand == null)
                                {
                                    if (previousReturn.Operand == null)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            node[indexLastStatement] = previousIf.Condition;
                                            node.RemoveAt(indexPrevious);
                                    ...
                                    else
                                    {
                                        ...
                                        node[indexLastStatement] = new ReturnStatement(null, m_parser, conditional);
                                        node.RemoveAt(indexPrevious);
                                        Optimize(conditional);
                                        somethingChanged = true;
                                    }
                            ...
                            else
                            {
                                ...
                                // and since we changed something, we need to bump the indexes down.
                                // since we removed a statement, the last index has gone down by one.
                                --indexLastStatement;
                                lastReturn = indexLastStatement < 0 ? null : node[indexLastStatement] as ReturnStatement;

                                // and we need to find the previous statement again
                                indexPrevious = PreviousStatementIndex(node, indexLastStatement);
                            }
                        }
                            ...
                            if (voidOperator != null && voidOperator.Operand is ConstantWrapper)
                            {
                                ...
                                if (voidOperator != null)
                                {
                                    if (isFunctionLevel)
                                    {
                                        ...
                                        node[indexLastStatement] = conditional.Condition;
                                    }
                                    else
                                    {
                                        ...
                                        node[indexLastStatement] = conditional.Condition;
                                        node.Append(new ReturnStatement(null, m_parser, null));
                                    }
                                }
                                else if (isFunctionLevel)
                                {
                                    ...
                                    var ifNode = new IfStatement(lastReturn.Context,
                                        ...
                                        new ReturnStatement(null, m_parser, conditional.TrueExpression),
                                        null);
                                    node[indexLastStatement] = ifNode;
                                }
                            }
                            else if (isFunctionLevel)
                            {
                                ...
                                if (voidOperator != null && voidOperator.Operand is ConstantWrapper)
                                {
                                    ...
                                    var ifNode = new IfStatement(lastReturn.Context,
                                        m_parser,
                                        conditional.Condition,
                                        new ReturnStatement(null, m_parser, conditional.FalseExpression),
                                        null);
                                    node[indexLastStatement] = ifNode;
                                }
                            }
                ...
                {
                    // if we pushed something onto the stack, pop it off now
                    if (node.LexicalEnvironment != null)
                    {
                        m_lexicalEnvironment.Pop();
                    }
                    if (node.VariableEnvironment != null)
                    {
                        m_variableEnvironment.Pop();
                    }
                }
            }
        }

        private static int PreviousStatementIndex(Block node, int index)
        {
            // get the index of the statement before the last return
            // (skip over function decls and importand comments)
            while (--index >= 0 && (node[index] is FunctionObject || node[index] is ImportantComment))
            {
                // the first test decrements the counter
            }
        ...
        public override void Visit(BreakStatement node)
        {
            if (node != null)
            {
                if (node.Label != null)
                {
                    // get the nest level from the label stack
                    var labelInfo = GetLabelInfo(node.Label);
                    if (labelInfo != null)
                    {
                        node.NestLevel = labelInfo.NestLevel;
                        ++labelInfo.ReferenceCount;
                    }
                    ...
                    {
                        // throw an error and remove the label
                        node.Context.HandleError(JSError.NoLabel, true);
                        node.Label = null;
                    }
                }
            ...
            if (node != null)
            {
                    ...
                    && m_parser.Settings.IsModificationAllowed(TreeModifications.StripDebugStatements))
                {
                    ...
                    if (member != null && member.IsDebuggerStatement && node.IsConstructor)
                    {
                        ...
                        var lookup = new Lookup("Object", node.Function.Context, m_parser);
                        m_lexicalEnvironment.Peek().ResolveLookup(lookup);
                        node.Function = lookup;

                        // and make sure the node list is empty
                        if (node.Arguments != null && node.Arguments.Count > 0)
                        {
                            node.Arguments = new AstNodeList(node.Arguments.Context, m_parser);
                        }
                    }
                }
                    ...
                    if (lookup != null)
                    {
                            ...
                            && m_parser.Settings.IsModificationAllowed(TreeModifications.NewObjectToObjectLiteral))
                        {
                            ...
                            if (node.Arguments == null || node.Arguments.Count == 0)
                            {
                                ...
                                if (node.ReplaceSelf(objLiteral))
                                {
                            ...
                            else if (node.Arguments.Count == 1)
                            {
                                ...
                                if (objectLiteral != null)
                                {
                                    ...
                                    node.ReplaceSelf(objectLiteral);
                            ...
                            && m_parser.Settings.IsModificationAllowed(TreeModifications.NewArrayToArrayLiteral))
                        {
                              ...
                              || (constWrapper != null && !constWrapper.IsNumericLiteral))
                            {
                                ...
                                if (node.ReplaceSelf(arrayLiteral))
                                {
                                    // recurse
                ...
                if (node.InBrackets && resourceStrings != null && resourceStrings.Count > 0)
                {
                    ...
                    // (same name)
                    // TODO: we will have already flagged this as undefined! Too late! move this to the referencing visitor?
                    Lookup rootLookup = node.Function as Lookup;
                    if (rootLookup != null
                        && rootLookup.Reference != null
                        && rootLookup.Reference.Base == null
                        && string.CompareOrdinal(rootLookup.Name, resourceStrings.Name) == 0)
                    {
                        ...
                        if (node.Arguments.Count == 1)
                        {
                            ...
                            if (argConstant != null)
                            {
                                ...
                                node.ReplaceSelf(resourceLiteral);
                    ...
                    if (argText != null)
                    {
                            ...
                            && !string.IsNullOrEmpty(newName = m_parser.Settings.GetNewName(argText)))
                        {
                            ...
                            if (m_parser.Settings.IsModificationAllowed(TreeModifications.BracketMemberToDotMember)
                                ...
                                && !JSScanner.IsKeyword(newName, m_lexicalEnvironment.Peek().UseStrict))
                            {
                                ...
                                node.ReplaceSelf(replacementMember);
                            ...
                            && !JSScanner.IsKeyword(argText, m_lexicalEnvironment.Peek().UseStrict))
                        {
                    ...
                    && m_parser.Settings.IsModificationAllowed(TreeModifications.DateGetTimeToUnaryPlus))
                {
                        ...
                        && dateConstructor.IsConstructor)
                    {
                        ...
                        if (lookup != null 
                            && lookup.Reference != null
                            && string.CompareOrdinal(lookup.Name, "Date") == 0
                            && lookup.Reference.Category == BindingCategory.Predefined)
                        {
                ...
                else
                {
                    ...
                    if (isEval)
                    {
                        if (m_parser.Settings.EvalTreatment != EvalTreatment.Ignore)
                        {
                            ...
                            var environment = m_lexicalEnvironment.Peek();
                            environment.IsKnownAtCompileTime = false;

                            if (m_parser.Settings.EvalTreatment == EvalTreatment.MakeAllSafe)
                            {
                                while ((environment = environment.Outer) != null)
                                {
                                    environment.IsKnownAtCompileTime = false;
                                }
                            }
        ...
        public override void Visit(DeleteNode node)
        {
            if (node != null)
            {
                ...
                if (m_lexicalEnvironment.Peek().UseStrict)
                {
        ...
        public override void Visit(FunctionObject node)
        {
            if (node != null)
            {
                try
                {
                    // push the function's lexical environment on both the lexical and the variable stacks
                    m_lexicalEnvironment.Push(node.LexicalEnvironment);
                    m_variableEnvironment.Push(node.LexicalEnvironment);

                    // if we are removing unneeded code,
                    // then process the lexical environment now, before we bother minifying all the
                    // code *within* those functions we'll just end up removing
                    if (m_parser.Settings.RemoveUnneededCode)
                    {
                        RemoveUnreferencedFunctionDeclarations(node.LexicalEnvironment);
                    }

                    if (node.FunctionType == FunctionType.Expression
                        && node.Binding != null 
                        && node.Binding.ReferenceCount == 0
                        && m_parser.Settings.RemoveFunctionExpressionNames)
                    {
                        // there was a name, and we have a binding, the name wasn't referenced anywhere,
                        // and we want to remove function expression names. Get rid of it.
                        node.Binding = null;
                        node.Name = string.Empty;
                    }

                    // let's check all the parameters for being referenced and remove any off the tail
                    // end of the list that aren't
                    if (m_parser.Settings.RemoveUnneededCode
                        && m_parser.Settings.IsModificationAllowed(TreeModifications.RemoveUnusedParameters))
                    {
                        if (node.ParameterDeclarations != null)
                        {
                            ...
                            {
                                var parameter = node.ParameterDeclarations[ndx];
                                if (parameter.Binding == null || parameter.Binding.ReferenceCount > 0)
                                {
                                    // found one with a reference (or no binding, which means we don't know if it's referenced)
                                    break;
                                }

                                // the parameter has a binding and no references -- get rid of it
                    ...
                    if (m_lexicalEnvironment.Peek().UseStrict)
                    {
                        ...
                        {
                            if (!Object.ReferenceEquals(node.IdContext, null))
                            {
                                node.IdContext.HandleError(JSError.StrictModeFunctionName, true);
                            }
                            else if (!Object.ReferenceEquals(node.Context, null))
                            {
                    ...
                    else if (node.ParameterDeclarations != null && node.ParameterDeclarations.Count > 0)
                    {
                        // not strict
                    ...
                    // by now the this-literals would have been identified for this function scope
                    if (m_parser.Settings.IsModificationAllowed(TreeModifications.CombineThisLiterals))
                    {
                        // gotta be at least four before it pays off
                        if (node.LexicalEnvironment.ThisLiterals != null
                            && node.LexicalEnvironment.ThisLiterals.Count >= 4)
                        {
                            // use an invalid name that we know needs to be renamed to something safe later
                            var binding = node.AddGeneratedVar("[this]", new ThisLiteral(null, m_parser));

                            // and replace each this-literal with a lookup to this variable
                            foreach (var thisLiteral in node.LexicalEnvironment.ThisLiterals)
                            {
                                // replace the literal with a lookup
                                var lookup = new Lookup(binding.Name, thisLiteral.Context, m_parser);
                                thisLiteral.ReplaceSelf(lookup);

                                // force the lookup to get resolved and a reference added
                                m_lexicalEnvironment.Peek().ResolveLookup(lookup);
                            }
                        }
                    }
        ...
        public override void Visit(IfStatement node)
        {
            if (node != null)
            {
                    ...
                    if (node.TrueBlock.Count == 1 && node.FalseBlock.Count == 1)
                    {
                        ...
                        if (trueReturn != null && trueReturn.Operand != null)
                        {
                            ...
                            if (falseReturn != null && falseReturn.Operand != null)
                            {
                                ...
                                // our if-node with it
                                var returnNode = new ReturnStatement(
                                    node.Context,
                                    m_parser,
                                    conditional);

                                node.ReplaceSelf(returnNode);
                    ...
                    && m_parser.Settings.IsModificationAllowed(TreeModifications.IfEmptyToExpression))
                {
                    ...
                    // the expression
                    node.ReplaceSelf(node.Condition);
        ...
        public override void Visit(LabeledStatement node)
        {
            if (node != null)
            {
                // create a label info object and push it on the stack
                var labelInfo = new LabelInfo() { Name = node.Label };
                if (m_labelStack == null)
                {
                    m_labelStack = new Stack<LabelInfo>();
                }
                m_labelStack.Push(labelInfo);

                // the nest level is determined by the number of items on the stack.
                // one-based: a zero for break or continue statements mean there is no
                // matched label definition.
                labelInfo.NestLevel = node.NestLevel = m_labelStack.Count;

                // recurse the statement
                base.Visit(node);

                // pop it off
                m_labelStack.Pop();

                // now, if no break or continue statement referenced this label, maybe we
                // should get rid of it
                if (labelInfo.ReferenceCount == 0
                    && m_parser.Settings.LocalRenaming != LocalRenaming.KeepAll
                    && m_parser.Settings.IsModificationAllowed(TreeModifications.RemoveUnnecessaryLabels))
                {
                    // replace the labeled statement with just the statement (sans-label)
                    node.ReplaceSelf(node.Statement);
                }
            }
        }
        ...
        public override void Visit(Lookup node)
        {
            if (node != null)
            {
                LexicalEnvironment scope = m_lexicalEnvironment.Peek();
                if (JSScanner.IsKeyword(node.Name, scope.UseStrict))
                {
                    node.Context.HandleError(JSError.KeywordUsedAsIdentifier, true);
                }

                // see if the reference is the the global scope
                if (node.Reference != null && node.Reference.Base == m_parser.GlobalScope)
                {
                    Binding binding;
                    if (node.Reference.Base.TryGetBinding(node.Name, out binding)
                        && binding.Category == BindingCategory.Predefined)
                    {
                        ...
                        if (string.CompareOrdinal(node.Name, "NaN") == 0)
                        {
                            ...
                            node.ReplaceSelf(new ConstantWrapper(double.NaN, PrimitiveType.Number, node.Context, m_parser));
                        }
                        else if (string.CompareOrdinal(node.Name, "Infinity") == 0)
                        {
                            // don't analyze the new ConstantWrapper -- we don't want it to take part in the
                            // duplicate constant combination logic should it be turned on.
                            node.ReplaceSelf(new ConstantWrapper(double.PositiveInfinity, PrimitiveType.Number, node.Context, m_parser));
                        }
                    }
                }
            }
            ...
            if (node != null)
            {
                ...
                if (resourceStrings != null && resourceStrings.Count > 0)
                {
                        ...
                        && string.CompareOrdinal(rootLookup.Name, resourceStrings.Name) == 0)
                    {
                        ...
                        node.ReplaceSelf(stringLiteral);
                ...
                if (JSScanner.IsKeyword(node.Name, m_lexicalEnvironment.Peek().UseStrict))
                {
            ...
            if (node != null)
            {
                    ...
                    && constantWrapper.IsNumericLiteral)
                {
                        ...
                        && m_parser.Settings.IsModificationAllowed(TreeModifications.ApplyUnaryMinusToNumericLiteral))
                    {
                        ...
                        if (node.ReplaceSelf(constantWrapper))
                        {
        ...
        public override void Visit(ReturnStatement node)
        {
            if (node != null)
            {
                // it makes no sense to have a return outside of a function, so check the VARIABLE environment.
                // it should always be either the global scope or a function scope.
                // TODO: unless we are crunching an implicit function (an "onEVENT" attribute's value in HTML source, for instance)
                LexicalEnvironment scope = m_variableEnvironment.Peek();
        ...
        public override void Visit(ThisLiteral node)
        {
            if (m_parser.Settings.IsModificationAllowed(TreeModifications.CombineThisLiterals))
            {
                // the variable environment should be either the global environment or
                // a function environment -- we only care if it's a function environment.
                var variableEnvironment = m_variableEnvironment.Peek() as DeclarativeEnvironment;
                if (variableEnvironment != null)
                {
                    // add this object to the list of this-literals
                    variableEnvironment.AddThisLiteral(node);
                }
            }
        }
        ...
        public override void Visit(TryStatement node)
        {
            if (node != null)
            {
                ...
                if (m_lexicalEnvironment.Peek().UseStrict && !string.IsNullOrEmpty(node.CatchVarName))
                {
        ...
        public override void Visit(VariableDeclaration node)
        {
            if (node != null)
            {
                ...
                bool isStrict = m_lexicalEnvironment.Peek().UseStrict;
                if (JSScanner.IsKeyword(node.Identifier, isStrict))
                {
                    node.IdentifierContext.HandleError(JSError.KeywordUsedAsIdentifier, true);
                }
                else if (isStrict 
        ...
        private static bool AreAssignmentsInVar(BinaryOperator binaryOp, VarStatement varStatement)
        {
        ...
        private static bool VarDeclExists(VarStatement node, int ndx, string name)
        {
        ...
        private static void DeleteNoInits(VarStatement node, int min, string name)
        {
        ...
        private LabelInfo GetLabelInfo(string labelName)
        {
            if (m_labelStack != null)
            {
                foreach (var label in m_labelStack)
                {
                    if (string.CompareOrdinal(labelName, label.Name) == 0)
                    {
                        return label;
                    }
                }
        ...
        private static void RemoveUnreferencedFunctionDeclarations(LexicalEnvironment environment)
        {
            // if nothing was defined, then there's nothing to do
            var defined = environment.DefinedWithin;
            if (defined != null)
            {
                // iterate over all the bindings, looking for those with FunctionObjects as their values,
                // those funcion objects are type Declaration, and the reference count is zero
                foreach (var binding in defined)
                {
                    FunctionObject funcObj;
                    if (binding.ReferenceCount == 0
                        && (funcObj = binding.Value as FunctionObject) != null
                        && funcObj.FunctionType == FunctionType.Declaration)
                    {
                        // whack it
                        funcObj.ReplaceSelf(null);
                    }
                }
            }
        }
