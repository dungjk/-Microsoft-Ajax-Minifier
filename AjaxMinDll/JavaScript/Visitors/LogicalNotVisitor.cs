...
namespace Microsoft.Ajax.Utilities.JavaScript.Visitors
{
    public class LogicalNot : TreeVisitor
    {
        ...
        {
            // DON'T use ReplaceSelf, because the act of changing the operand to below
            // to the NumericUnary changes its parent, so by the time we get to ReplaceSelf,
            // the parent is the numericUnary, not whatever it is now.
            operand.Parent.ReplaceChild(operand, new NumericUnary(
                null,
                m_parser,
                operand,
                JSToken.LogicalNot));
