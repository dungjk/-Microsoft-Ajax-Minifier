    ...
    public sealed class CallNode : Expression
    {
        ...
        public AstNode Function
        {
            ...
            set
            {
                if (value != m_func)
                {
                    if (m_func != null && m_func.Parent == this)
                    {
                        m_func.Parent = null;
                    }
                    m_func = value;
        ...
        public AstNodeList Arguments
        {
            ...
            set
            {
                if (value != m_args)
                {
                    if (m_args != null && m_args.Parent == this)
                    {
                        m_args.Parent = null;
                    }
                    m_args = value;
        ...
        public override bool ReplaceChild(AstNode oldNode, AstNode newNode)
        {
            ...
            {
                Arguments = newNode as AstNodeList;
                return true;
            }
            return false;
        }
        ...
        {
            // if the target is the function portion of the call, then ask the call's
            // parent to find a name. Otherwise ask the function portion of the call.
            string funcName = (target == m_func ? Parent : m_func).GetFunctionNameGuess(this);
            ...
            // so there should be two parameters, and our target should be the second.
            if (funcName == "addMethod" && m_args.Count == 2 && target == m_args[1])
            {
                // the first one should be a string constant
                ConstantWrapper firstParam = m_args[0] as ConstantWrapper;
                if ((firstParam != null) && firstParam.IsStringLiteral)
                {
