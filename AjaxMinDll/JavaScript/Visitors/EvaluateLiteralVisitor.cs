...
namespace Microsoft.Ajax.Utilities.JavaScript.Visitors
{
    internal class EvaluateLiteralVisitor : TreeVisitor
    {
        ...
        public static void Evaluate(Block program, JSParser parser)
        {
            var visitor = new EvaluateLiteralVisitor(parser);
            program.Accept(visitor);
        }

        private EvaluateLiteralVisitor(JSParser parser) 
        {
            m_parser = parser;
        }
                ...
                if (parentCall != null && parentCall.InBrackets)
                {
                        ...
                        && !string.IsNullOrEmpty(newName = m_parser.Settings.GetNewName(combinedString)))
                    {
                        ...
                        if (m_parser.Settings.IsModificationAllowed(TreeModifications.BracketMemberToDotMember)
                            ...
                            && !JSScanner.IsKeyword(newName, parentCall.EnclosingLexicalEnvironment.UseStrict))
                        {
                            ...
                            parentCall.ReplaceSelf(replacementMember);
                            return true;
                        }
                        else
                        {
                    ...
                    else if (m_parser.Settings.IsModificationAllowed(TreeModifications.BracketMemberToDotMember))
                    {
                        ...
                        if (JSScanner.IsSafeIdentifier(combinedString) && !JSScanner.IsKeyword(combinedString, parentCall.EnclosingLexicalEnvironment.UseStrict))
                        {
        ...
        private void EvalThisOperator(BinaryOperator node, ConstantWrapper left, ConstantWrapper right)
        {
            ...
            switch (node.OperatorToken)
            {
                ...
                case JSToken.Divide:
                    ...
                    if (newLiteral != null && OutputVisitor.Output(newLiteral).Length > OutputVisitor.Output(node).Length)
                    {
        ...
        private void EvalToTheLeft(BinaryOperator node, ConstantWrapper thisConstant, ConstantWrapper otherConstant, BinaryOperator leftOperator)
        {
                ...
                || (leftOperator.OperatorToken == JSToken.Divide && node.OperatorToken == JSToken.Multiply))
            {
                if (m_parser.Settings.IsModificationAllowed(TreeModifications.EvaluateNumericExpressions))
                {
                    ...
                    // get the lengths
                    int otherOverThisLength = otherOverThis != null ? OutputVisitor.Output(otherOverThis).Length : int.MaxValue;
                    int thisOverOtherLength = thisOverOther != null ? OutputVisitor.Output(thisOverOther).Length : int.MaxValue;
                        ...
                        && (thisOverOther == null || otherOverThisLength < thisOverOtherLength))
                    {
                        ...
                        if (otherOverThisLength <= OutputVisitor.Output(otherConstant).Length + OutputVisitor.Output(thisConstant).Length + 1)
                        {
                            // same operator
                    ...
                    {
                        // but only if it's smaller than the original expression
                        if (thisOverOtherLength <= OutputVisitor.Output(otherConstant).Length + OutputVisitor.Output(thisConstant).Length + 1)
                        {
                            // opposite operator
        ...
        private void EvalFarToTheLeft(BinaryOperator node, ConstantWrapper thisConstant, ConstantWrapper otherConstant, BinaryOperator leftOperator)
        {
            ...
            else if (node.OperatorToken == JSToken.Divide)
            {
                if (leftOperator.OperatorToken == JSToken.Divide)
                {
                    ...
                    if (newLiteral != null && NoMultiplicativeOverOrUnderFlow(otherConstant, thisConstant, newLiteral)
                        && OutputVisitor.Output(newLiteral).Length <= OutputVisitor.Output(thisConstant).Length + OutputVisitor.Output(otherConstant).Length + 1)
                    {
                        RotateFromRight(node, leftOperator, newLiteral);
                    }
                }
                else if (leftOperator.OperatorToken == JSToken.Multiply)
                {
                    ...
                    if (otherOverThis != null && NoMultiplicativeOverOrUnderFlow(otherConstant, thisConstant, otherOverThis)
                        && (thisOverOther == null || otherOverThisLength < thisOverOtherLength))
                    {
                        if (otherOverThisLength <= OutputVisitor.Output(thisConstant).Length + OutputVisitor.Output(otherConstant).Length + 1)
                        {
                    ...
                    {
                        if (thisOverOtherLength <= OutputVisitor.Output(thisConstant).Length + OutputVisitor.Output(otherConstant).Length + 1)
                        {
                            // swap the operands
        ...
        private void EvalToTheRight(BinaryOperator node, ConstantWrapper thisConstant, ConstantWrapper otherConstant, BinaryOperator rightOperator)
        {
            ...
            else if (node.OperatorToken == JSToken.Divide)
            {
                ...
                else if (rightOperator.OperatorToken == JSToken.Divide)
                {
                    ...
                    int leftOverRightLength = leftOverRight != null ? OutputVisitor.Output(leftOverRight).Length : int.MaxValue;
                    int rightOverLeftLength = rightOverLeft != null ? OutputVisitor.Output(rightOverLeft).Length : int.MaxValue;
                        ...
                        && (rightOverLeft == null || leftOverRightLength < rightOverLeftLength))
                    {
                        ...
                        if (leftOverRightLength <= OutputVisitor.Output(thisConstant).Length + OutputVisitor.Output(otherConstant).Length + 1)
                        {
                    ...
                    {
                        // but only if the resulting value is smaller than the original expression
                        if (rightOverLeftLength <= OutputVisitor.Output(thisConstant).Length + OutputVisitor.Output(otherConstant).Length + 1)
                        {
                ...
                && m_parser.Settings.IsModificationAllowed(TreeModifications.EvaluateNumericExpressions))
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            left.ReplaceSelf(new ConstantWrapper(leftValue, PrimitiveType.Number, left.Context, m_parser));
                        }
                        if (!right.IsNumericLiteral && ConstantWrapper.NumberIsOkayToCombine(rightValue))
                        {
                            right.ReplaceSelf(new ConstantWrapper(rightValue, PrimitiveType.Number, right.Context, m_parser));
                        }
                    }
                }
                        ...
                        if (left != null)
                        {
                            if (node.OperatorToken == JSToken.Comma)
                            {
                                ...
                                if (rightConstant != null)
                                {
                                    ...
                                    {
                                        node.ReplaceSelf(rightConstant);
                                    }
                                }
                                else
                                {
                                    // replace the comma operator with the right-hand operand
                                    node.ReplaceSelf(node.Operand2);
                                }
                            }
                            else
                            {
                ...
                if (m_parser.Settings.IsModificationAllowed(TreeModifications.EvaluateNumericExpressions))
                {
                    ...
                    if (literalCondition != null)
                    {
                        try
                        {
                            ...
                            node.ReplaceSelf(literalCondition.ToBoolean() ? node.TrueExpression : node.FalseExpression);
                        }
                        catch (InvalidCastException)
                        {
                        ...
                        switch (node.OperatorToken)
                        {
                            case JSToken.Plus:
                                try
                                {
                                    ...
                                    node.ReplaceSelf(new ConstantWrapper(literalOperand.ToNumber(), PrimitiveType.Number, node.Context, m_parser));
                                }
                                catch (InvalidCastException)
                                {
                                    // some kind of casting in ToNumber caused a situation where we don't want
                            ...
                            case JSToken.BitwiseNot:
                                try
                                {
                                    ...
                                    node.ReplaceSelf(new ConstantWrapper(Convert.ToDouble(~literalOperand.ToInt32()), PrimitiveType.Number, node.Context, m_parser));
                                }
                                catch (InvalidCastException)
                                {
                                    // some kind of casting in ToNumber caused a situation where we don't want
                            ...
                            case JSToken.LogicalNot:
                                ...
                                try
                                {
                                    node.ReplaceSelf(new ConstantWrapper(!literalOperand.ToBoolean(), PrimitiveType.Boolean, node.Context, m_parser));
                                }
                                catch (InvalidCastException)
                                {
                                    // ignore any invalid cast exceptions
            ...
            if (node != null)
            {
                ...
                if (m_parser.Settings.IsModificationAllowed(TreeModifications.EvaluateNumericExpressions))
                {
                    ...
                    if (literalOperand != null)
                    {
                        ...
                        {
                            node.ReplaceSelf(new ConstantWrapper(typeName, PrimitiveType.String, node.Context, m_parser));
                        }
                    }
                    else if (node.Operand is ObjectLiteral)
                    {
                        node.ReplaceSelf(new ConstantWrapper("object", PrimitiveType.String, node.Context, m_parser));
                    }
                }
            }
        }
        ...
        public override void Visit(VoidNode node)
        {
            if (node != null)
            {
                ...
                {
                    // see if our operand is a ConstantWrapper
                    if (node.Operand is ConstantWrapper)
                    {
                        // either number, string, boolean, or null.
                            ...
                            if (isTrue)
                            {
                                ...
                                if (m_parser.Settings.IsModificationAllowed(TreeModifications.MoveVarIntoFor))
                                {
                                    ...
                                    if (parentBlock != null)
                                    {
                                        ...
                                        if (whileIndex > 0)
                                        {
                                            VarStatement previousVar = parentBlock[whileIndex - 1] as VarStatement;
                                            if (previousVar != null)
                                            {
                                                // the previous statement is a var, and we're going to convert the current
                                                // while-statement into a for-statement with no initializer.
                                                // we _should_ be able to move the previous var into the new for-statement's
                                                // initializer.
                                                initializer = previousVar;

                                                // this could cause problems with the iteration of parent items it the
                                                // iterator doesn't take a snapshot before calling each child.
                                                // If the current item in the block is n, and we
                                                // remove n-1, then the current item becomes n-1, and the next
                                                // block item will be n.... so when we move to n+1 the next time around,
                                                // we've skipped n, which was n+1, and go to what was n+2. 
                                ...
                                ForStatement forNode = new ForStatement(node.Context, m_parser, initializer, null, null, node.Body);
                                node.ReplaceSelf(forNode);
                            }
                            else if (constantCondition.IsNotOneOrPositiveZero)
                            {
