...
{
    internal class NormalizeVisitor : TreeVisitor
    {
        ...
        private List<VarStatement> m_varStatements;
        ...
        public static void Apply(Block block, CodeSettings settings)
        {
            // if no settings were passed, create a default object 
            settings = settings ?? new CodeSettings();

            // create a new instance of the visitor and apply it to the block
            var visitor = new NormalizeVisitor(settings);
            ...
            // AND it has any initializers,
            // then just leave it alone because we will only add bytes by moving it around
            if (visitor.m_varStatements != null 
                && visitor.m_varStatements.Count > 0
                && (visitor.m_varStatements.Count > 1 || !visitor.m_varStatements[0].HasAnyInitializers))
            {
            ...
            if (visitor.m_functionExpressions != null)
            {
                ...
                {
                    Apply(funcExpr.Body, settings);
                }
            }
        }

        private NormalizeVisitor(CodeSettings settings)
        {
        ...
        private static int RelocateFunction(Block block, int insertAt, FunctionObject funcDecl)
        {
            if (block[insertAt] != funcDecl)
            {
                ...
                {
                    // NOTE: we'll be removing an item in the parent block, but we'll be adding it back in
                    // at an earlier location. So the position of later nodes in the parent will remain the same.
                    // remove the function from it's parent, which will take it away from where it is right now.
                    funcDecl.ReplaceSelf(null);
        ...
        private static int RelocateVar(Block block, int insertAt, VarStatement varStatement)
        {
            ...
            if (block[insertAt] != varStatement && !InsideConditionalComment(varStatement))
            {
                ...
                var existingVar = block[insertAt] as VarStatement;
                ...
                else
                {
                    ...
                    {
                        // save the lexical context for the var statement, because that's where
                        // we'll need to resolve the new lookup we create
                        var varEnvironment = varStatement.EnclosingLexicalEnvironment;
                        ...
                        for (var ndx = 0; ndx < varStatement.Count; ++ndx)
                        {
                            ...
                            if (varDecl.Initializer != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    var lookup = new Lookup(varDecl.Identifier, varDecl.IdentifierContext, varDecl.Parser);
                                    varEnvironment.ResolveLookup(lookup);
                        ...
                        if (assignments.Count > 0)
                        {
                            ...
                            // place next.
                            varStatement.ReplaceSelf(expression);
                        }
                        else
                        {
                            // no initializers.
                            // if the parent is a for-in statement...
                            var forInParent = varStatement.Parent as ForEachStatement;
                            if (forInParent != null)
                            {
                                // we want to replace the var statement with a lookup for the var
                                // there should be only one vardecl
                                var varDecl = varStatement[0];
                                var lookup = new Lookup(varDecl.Identifier, varDecl.IdentifierContext, varStatement.Parser);
                                block.EnclosingLexicalEnvironment.ResolveLookup(lookup);

                                varStatement.ReplaceSelf(lookup);
                            }
                            else
                            {
                                // just remove the var statement altogether
                                // we're not iterating over the parent's children, so it should be okay.
                                varStatement.ReplaceSelf(null);
                            }
                        }
            ...
            if (node != null)
            {
                ...
                if (m_combineAdjacentVars)
                {
                    ...
                    for (int ndx = node.Count - 1; ndx > 0; --ndx)
                    {
                        ...
                        if (previousVar != null && node[ndx] is VarStatement)
                        {
        ...
        public override void Visit(VarStatement node)
        {
            if (node != null)
            {
                ...
                if (m_moveVarStatements)
                {
                    ...
                    {
                        m_varStatements = new List<VarStatement>();
                    }
