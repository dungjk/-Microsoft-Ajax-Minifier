...
namespace Microsoft.Ajax.Utilities
{
    public sealed class XmlScopeReport : IScopeReport
    {
            ...
            if (globalScope != null)
            {
                ...
                if (globalScope.UndefinedReferences != null && globalScope.UndefinedReferences.Count > 0)
                {
                    ...
                    foreach (var undefined in globalScope.UndefinedReferences)
                    {
                        ...
                        m_writer.WriteAttributeString("srcLine", undefined.Line.ToString(CultureInfo.InvariantCulture));
                        m_writer.WriteAttributeString("srcCol", (undefined.Column + 1).ToString(CultureInfo.InvariantCulture));
        ...
        private void ProcessScope(ActivationObject scope)
        {
            ...
            if (functionScope != null)
            {
                ...
                if (functionObject != null)
                {
                    ...
                    {
                        m_writer.WriteAttributeString("srcLine", functionObject.Context.StartLineNumber.ToString(CultureInfo.InvariantCulture));
                        m_writer.WriteAttributeString("srcCol", (functionObject.Context.StartColumn + 1).ToString(CultureInfo.InvariantCulture));
                    }

                    if (m_useReferenceCounts && functionObject.VariableField != null)
                    {
                        var refCount = functionObject.VariableField.RefCount;
                        m_writer.WriteAttributeString("refcount", refCount.ToString(CultureInfo.InvariantCulture));
                    ...
                    if (functionObject.ParameterDeclarations != null)
                    {
                        foreach (var parameter in functionObject.ParameterDeclarations)
                        {
                            ...
                            {
                                m_writer.WriteAttributeString("srcLine", parameter.Context.StartLineNumber.ToString(CultureInfo.InvariantCulture));
                                m_writer.WriteAttributeString("srcCol", (parameter.Context.StartColumn + 1).ToString(CultureInfo.InvariantCulture));
                            }

                            if (m_useReferenceCounts && parameter.Field != null)
                            {
                                m_writer.WriteAttributeString("refcount", parameter.Field.RefCount.ToString(CultureInfo.InvariantCulture));
                            }
            ...
            else
            {
                ...
                if (catchScope != null)
                {
                    ...
                    {
                        m_writer.WriteAttributeString("srcLine", catchVariable.OriginalContext.StartLineNumber.ToString(CultureInfo.InvariantCulture));
                        m_writer.WriteAttributeString("srcCol", (catchVariable.OriginalContext.StartColumn + 1).ToString(CultureInfo.InvariantCulture));
                    }

                    if (m_useReferenceCounts)
                    {
                        m_writer.WriteAttributeString("refcount", catchVariable.RefCount.ToString(CultureInfo.InvariantCulture));
                    }
        ...
        private void ProcessField(JSVariableField field, bool isDefined)
        {
            ...
            {
                m_writer.WriteAttributeString("srcLine", field.OriginalContext.StartLineNumber.ToString(CultureInfo.InvariantCulture));
                m_writer.WriteAttributeString("srcCol", (field.OriginalContext.StartColumn + 1).ToString(CultureInfo.InvariantCulture));
            }
